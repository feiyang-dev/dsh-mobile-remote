...
Write-Host "== 3/4 推送到 GitHub（含 v1.45 tag，触发自动发布 Actions）==" -ForegroundColor Cyan
git push -u origin master
git push origin v1.45

Write-Host "== 4/4 完成 ==" -ForegroundColor Green
Write-Host "推送完成！GitHub Actions 将自动执行：npm run check → npm publish → 生成 Release(v1.45)"
Write-Host ""
