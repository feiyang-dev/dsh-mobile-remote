...
//   - 心跳上报、状态查询、停止
//
// 运行时依赖：Node 内置模块 + `ws`（WebSocket 长连接接收中转服务状态推送，
// 替代高频轮询；ws 依赖缺失时自动回退 HTTP 低频查询）。frpc 二进制由中转服务提供或用户本地放置。
//
...
import { createWsStatusClient } from './ws-client.js'
...
async function startExternalInner(state, body, patchFile, defaultLocalPort) {
  ...
  return await new Promise((resolve) => {
    ...
    const confirmOnline = () => {
      ...
      // 建立 WS 长连接接收隧道状态推送（替代每 5s HTTP 轮询中转服务器）；
      // ws 依赖缺失（离线安装）时 wsClient.connect() 自动 no-op，心跳走 HTTP 兜底
      state.serverStatus = null
      if (state.wsClient) {
        try { state.wsClient.disconnect() } catch { /* ignore */ }
      }
      state.wsClient = createWsStatusClient({
        serverBase: base,
        bindCode: state.bindCode,
        onStatus: (data) => { state.serverStatus = data },
        logger: null,
      })
 ...
 *  优先经 WS 长连接发送（零 HTTP 请求）；WS 不可用/未连上时回退 HTTP。 */
async function sendHeartbeat(state) {
  ...
  if (!bindCode) return
  const status = state.status === 'online' ? 'online' : 'offline'
  // WS 已连接 → 直接经长连接发送，减轻中转服务器 HTTP 压力
  if (state.wsClient && state.wsClient.isOpen() && state.wsClient.sendHeartbeat(status)) {
    return
  }
  // WS 不可用 / 未连上 → HTTP 心跳兜底（旧后端 / ws 依赖缺失场景）
  try {
    await fetch(`${base.replace(/\/$/, '')}/api/tunnel/heartbeat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ bindCode, status }),
    })
  } catch { /* ignore */ }
}
