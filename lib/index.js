...
import { wsAvailable } from './ws-client.js'
...
function runtimeInfo() {
  // 采集时刻：前端可据此在两次 /status 轮询之间本地累加，让"运行时长"每秒跳动。
  const at = Date.now()
 ...
 *  额外返回 service 摘要（服务名 / 本机主机名 / dsh 版本 / 当前访问域名），
 *  供密码门禁界面展示"当前是谁在提供服务"等更多信息。 */
function handleAuthStatus() {
  return async (req, res) => {
    json(res, 200, {
      ...
      // 服务信息：密码界面上展示（"服务者"以本机主机名标识）
      service: {
        name: 'DeepSeek Harness',
        hostname: hostname(),
        version: detectDshVersion() || '',
...
/**
 * POST /__dsh_remote/set-password { password, currentPassword }（仅本机/内网来源可用）：
 * 设置或修改远程访问密码。远程来源被拒绝，防止外部任意篡改。
 * 安全要求：已设置过密码时，修改或清除都必须携带 currentPassword 并验证通过，
 * 防止"改密码/清密码"接口被无鉴权滥用。
 */
function handleSetPassword() {
  return async (req, res) => {
    ...
    // 已设置密码时：修改 / 清除都必须先验证当前密码（防未授权篡改门禁）
    if (isEnabled()) {
      if (!verifyPassword(current)) {
        json(res, 401, { ok: false, code: 'BAD_CURRENT_PASSWORD', message: '当前密码不正确，无法修改或清除' })
        return
      }
    }
    // 密码强度检查
 ...
 * 仅作 WS 推送不可用（ws 依赖缺失 / 旧后端）时的 HTTP 兜底。
 * 优先调用 /api/tunnel/stats（含 frps 实时流量/连接数等），接口不可用则回退到
 * /api/tunnel/status（旧版后端）。
 * 注意：带 60s 节流缓存——即使前端每 5s 轮询本机 /status，也不会高频打中转服务器。
 */
let serverStatusCache = { at: 0, value: null }
async function queryServerStatus(base, bindCode) {
  if (!bindCode) return null
  const now = Date.now()
  if (serverStatusCache.value && now - serverStatusCache.at < 60000) {
    return serverStatusCache.value
  }
  ...
  const data = stats || await call(`/api/tunnel/status${q}`)
  serverStatusCache = { at: now, value: data }
  return data
}
...
function handleStatus(ctx, devices, external, tracker, dshCache) {
  return async (req, res) => {
    ...
    // 外网隧道在线时，优先读 WS 推送缓存（经长连接实时更新，零中转服务器请求）；
    // 仅当从未收到推送（ws 依赖缺失 / WS 未建立）时才回退 HTTP 低频查询（60s 节流）。
    let server = null
    let serverSource = 'none'
    if (extRunning && persisted) {
      if (external.serverStatus) {
        server = external.serverStatus
        serverSource = 'ws'
      } else {
    ...
    // WS 长连接是否已建立（供前端判断当前处于"实时推送"还是"退让/重连"）
    const wsConnected = !!(external.wsClient && external.wsClient.isOpen())
    json(res, 200, {
      ...
      external: {
        ...
        server,
        // WS 状态推送通道信息：让前端明确当前数据是 WS 实时推送还是 HTTP 兜底退让
        ws: {
          // ws 依赖是否可用（离线安装未带依赖时为 false，自动退让 HTTP）
          available: wsAvailable(),
          // WS 长连接是否已建立
          connected: wsConnected,
          // 当前 server 数据来源：'ws'（实时推送缓存）/ 'http'（兜底查询）/ 'none'
          source: serverSource,
          // 最近一次收到 WS 推送的时刻（未收到过为 null）
          lastPushAt: external.serverStatus && external.serverStatus._at
            ? external.serverStatus._at
            : null,
        },
      },
      // 远程访问密码门禁状态（供设置面板展示）
...
const GATE_CSS = `
/* DeepSeek 官网风格：深蓝黑底 + 极简白字 + 细边框 + 下划线输入 + 大留白 */
#dsh-remote-gate {
...
/* 背景光斑纹理（参考官网深蓝氛围） */
#dsh-remote-gate::before {
  content: '';
  position: absolute; inset: 0;
  background:
    radial-gradient(800px 500px at 18% 8%, rgba(70, 100, 170, 0.12), transparent 60%),
    radial-gradient(700px 420px at 88% 92%, rgba(50, 70, 140, 0.10), transparent 60%);
  pointer-events: none;
}
#dsh-remote-gate .gate-card {
  ...
  border-radius: 14px; padding: 44px 36px 30px;
  backdrop-filter: blur(10px);
}
/* Logo 组合：FishLogo + 文字 "deepseek | Harness"，胶囊圆角条（复刻官网 logo 与药丸按钮） */
#dsh-remote-gate .gate-logo-wrap {
...
#dsh-remote-gate .gate-logo {
  width: 26px; height: 19px; color: #fff; display: block; flex: none;
}
#dsh-remote-gate .gate-wordmark {
  font-size: 18px; line-height: 1; letter-spacing: .3px;
  display: flex; align-items: center; gap: 8px;
}
#dsh-remote-gate .gate-wordmark .w-name { color: #fff; font-weight: 500; }
#dsh-remote-gate .gate-wordmark .w-sep { color: #555; font-weight: 300; }
#dsh-remote-gate .gate-wordmark .w-prod { color: #d0d0d0; font-weight: 400; }
/* 标题 + 副标题 */
#dsh-remote-gate .gate-title {
  font-size: 16px; font-weight: 600; margin: 0 0 8px; text-align: center; letter-spacing: .2px;
}
#dsh-remote-gate .gate-sub {
  font-size: 13px; color: #8a8a8a; margin: 0 0 28px; line-height: 1.6; text-align: center;
}
/* 服务信息（访问地址 / 提供服务者 / 版本）：细边框、黑白灰 */
#dsh-remote-gate .gate-service {
  margin: 0 0 24px; padding: 12px 14px;
  border: 1px solid rgba(255,255,255,.12);
  border-radius: 10px;
  font-size: 12px; color: #8a8a8a; line-height: 1.9;
}
#dsh-remote-gate .gate-service .sv-row {
  display: flex; justify-content: space-between; align-items: baseline; gap: 12px; min-width: 0;
}
#dsh-remote-gate .gate-service .sv-label { flex: none; color: #6a6a6a; }
#dsh-remote-gate .gate-service .sv-value {
  flex: 1; min-width: 0; text-align: right; color: #e8e8e8;
  word-break: break-all; overflow-wrap: anywhere; font-weight: 500;
}
/* 圆角输入框：深色底 + 细白边 + 圆角，与整体圆角风格一致 */
#dsh-remote-gate .gate-input {
  width: 100%; box-sizing: border-box;
  padding: 11px 13px; font-size: 14px; color: #fff;
  background: rgba(255,255,255,.05);
  border: 1px solid rgba(255,255,255,.18);
  border-radius: 10px; outline: none;
  transition: border-color .15s ease, background .15s ease;
}
#dsh-remote-gate .gate-input:focus { border-color: #fff; background: rgba(255,255,255,.08); }
#dsh-remote-gate .gate-input::placeholder { color: #555; }
/* 主按钮：白底黑字（强对比 CTA） */
...
#dsh-remote-gate .gate-btn:hover { background: #e8e8e8; }
#dsh-remote-gate .gate-btn:disabled { opacity: .5; cursor: default; }
/* 加载动画：提交后按钮内旋转 spinner + 文案切换 */
#dsh-remote-gate .gate-spinner {
  display: none; width: 15px; height: 15px; flex: none;
  border: 2px solid rgba(0,0,0,.25); border-top-color: #000;
  border-radius: 50%; box-sizing: border-box;
  animation: dsh-gate-spin .7s linear infinite;
}
#dsh-remote-gate .gate-btn.gate-loading .gate-spinner { display: inline-block; }
@keyframes dsh-gate-spin { to { transform: rotate(360deg); } }
#dsh-remote-gate .gate-err {
  font-size: 12px; color: #ff5f56; margin-top: 10px; min-height: 16px; line-height: 1.4;
}
#dsh-remote-gate .gate-foot {
  margin-top: 20px; font-size: 11px; color: #555; text-align: center; letter-spacing: .2px;
}
`
...
(function () {
  ...
  function submit(g, input, btn) {
    ...
    var btnText = btn.querySelector('.gate-btn-text');
    // 立即进入加载态：禁用按钮 + spinner 旋转 + 文案切换，让用户明确知道正在进入
    btn.disabled = true; btn.classList.add('gate-loading');
    if (btnText) btnText.textContent = '正在进入…';
    fetch('/__dsh_remote/auth', {
      ...
      if (btnText) btnText.textContent = '进入';
